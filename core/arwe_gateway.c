...
static int gw_register_single_route(int fd, const char *name, const char *path,
  ...
  if (cfg->route_rl[0]) {
    len = snprintf(payload, sizeof(payload),
                   "%s %s %s %s %s proxy=http://%s:%u type=proxy rl=%s", name,
                   path, "GET", cfg->route_host, cfg->route_mode,
                   cfg->server_bind, cfg->server_port, cfg->route_rl);
  } else {
    len = snprintf(payload, sizeof(payload),
                   "%s %s %s %s %s proxy=http://%s:%u type=proxy", name, path,
                   "GET", cfg->route_host, cfg->route_mode, cfg->server_bind,
                   cfg->server_port);
  }
  ...
  if (ar_ipc_recv_frame(fd, (int *)&type, ack, &alen) != 0)
    return -1;
  if (type != IPC_ACK)
    return -1;
...
static int is_localhost_domain(const char *host) {
  ...
  if (len > 10 && strcasecmp(host + len - 10, ".localhost") == 0)
    return 1;
  return 0;
}
...
static int gw_register(int fd, const char *name, const arwe_gw_cfg_t *cfg,
  ...
  /* Suporta múltiplos hosts separados por vírgula ou espaço (ex:
   * "alrigroup.com, localhost") */
  char *p = hosts_buf;
  while (*p) {
    while (*p == ' ' || *p == '\t' || *p == ',')
      p++;
...
static void handle_query(int fd, arwe_server_t *server, const char *q,
  ...
  if (strcmp(cmd, "help") == 0 || strcmp(cmd, "--help") == 0 ||
    ...
    rlen = snprintf(resp, sizeof(resp),
                    ...
                    "active route count\n"
                    "  routes                - List all embedded .arweb static "
                    "views and endpoints\n"
