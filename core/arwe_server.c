...
typedef struct arwe_conn {
    int   fd;
    arwe_conn_state_t state;
    unsigned char rbuf[ARWE_SERVER_RECV_BUF];
    size_t rlen;
    ...
    struct arwe_conn *next;
} arwe_conn_t;

#define ARWE_SERVER_WORKERS 28
#define ARWE_SERVER_QUEUE_SIZE 16384
...
struct arwe_server {
    ...
    uint16_t port;
    arwe_route_t routes[ARWE_SERVER_MAX_ROUTES];
    int route_count;
    arwe_route_t notfound_route;
    int has_notfound_route;
    void *queue_mutex;
    void *queue_cond;
    arwe_work_item_t queue[ARWE_SERVER_QUEUE_SIZE];
    int queue_head;
    int queue_tail;
    int queue_count;
    void *workers[ARWE_SERVER_WORKERS];
};

static const char *cache_header(arwe_cache_t c) {
    switch (c) {
        case ARWE_CACHE_IMMUTABLE:
            return "public, max-age=31536000, immutable";
        case ARWE_CACHE_NO_CACHE:
            return "no-cache";
        default:
            return "no-store";
    }
...
arwe_server_t *arwe_server_create(void) {
    arwe_server_t *s = (arwe_server_t *)calloc(1, sizeof(*s));
    if (!s) return NULL;
    s->listening_fd = -1;
...
int arwe_server_add_route(arwe_server_t *s, const char *path,
    ...
    if (s->route_count >= ARWE_SERVER_MAX_ROUTES) return -1;
    arwe_route_t *r = &s->routes[s->route_count];
    r->path = path;
    r->data = data;
    r->size = size;
...
int arwe_server_load_arweb(arwe_server_t *s, const arwe_unit_t *unit,
    ...
    int n = arwe_pack_index(arweb, arweb_len, views, ARWE_ARWEB_MAX_SECTIONS);
    if (n <= 0) return -1;
    ...
    for (int i = 0; i < n; i++) {
        if (strcmp(views[i].name, "app.html") == 0) {
            if (is_404_unit) {
                ...
                nf.cache = ARWE_CACHE_NO_CACHE;
                arwe_server_set_notfound_route(s, &nf);
                arwe_server_add_route(s, "/notfound", views[i].data, views[i].size,
                                      "text/html; charset=utf-8",
                                      ARWE_CACHE_NO_CACHE);
                arwe_server_add_route(s, "/404", views[i].data, views[i].size,
                                      "text/html; charset=utf-8",
                                      ARWE_CACHE_NO_CACHE);
            } else {
                arwe_server_add_route(s, strdup(route), views[i].data, views[i].size,
                                      "text/html; charset=utf-8",
                                      ARWE_CACHE_NO_CACHE);
                if (strcmp(route, "/") == 0) {
                    arwe_server_add_route(s, "/", views[i].data, views[i].size,
                                          "text/html; charset=utf-8",
                                          ARWE_CACHE_NO_CACHE);
                }
            }
            ...
            arwe_server_add_route(s, "/mod/main.wasm", views[i].data,
                                  views[i].size, "application/wasm",
                                  ARWE_CACHE_IMMUTABLE);
            arwe_server_add_route(s, strdup(wasm_route), views[i].data, views[i].size,
                                  "application/wasm", ARWE_CACHE_IMMUTABLE);
        } else if (strcmp(views[i].name, "bundle.js") == 0 || strcmp(views[i].name, "main.js") == 0) {
            if (strcmp(route, "/") == 0) {
                char unit_js[128];
                snprintf(unit_js, sizeof(unit_js), "/%s.js", unit->name);
                arwe_server_add_route(s, strdup(unit_js), views[i].data, views[i].size,
                                      "application/javascript; charset=utf-8", ARWE_CACHE_IMMUTABLE);
                arwe_server_add_route(s, "/bundle.js", views[i].data, views[i].size,
                                      "application/javascript; charset=utf-8", ARWE_CACHE_IMMUTABLE);
                arwe_server_add_route(s, "/main.js", views[i].data, views[i].size,
                                      "application/javascript; charset=utf-8", ARWE_CACHE_IMMUTABLE);
            } else {
                char scoped_js[256];
                snprintf(scoped_js, sizeof(scoped_js), "%s/main.js", route);
                arwe_server_add_route(s, strdup(scoped_js), views[i].data, views[i].size,
                                      "application/javascript; charset=utf-8", ARWE_CACHE_IMMUTABLE);
            ...
            if (strcmp(route, "/") == 0) {
                ...
                arwe_server_add_route(s, strdup(unit_css), views[i].data, views[i].size,
                                      "text/css; charset=utf-8", ARWE_CACHE_IMMUTABLE);
                arwe_server_add_route(s, "/main.css", views[i].data, views[i].size,
                                      "text/css; charset=utf-8", ARWE_CACHE_IMMUTABLE);
                arwe_server_add_route(s, "/style.css", views[i].data, views[i].size,
                                      "text/css; charset=utf-8", ARWE_CACHE_IMMUTABLE);
            } else {
                char scoped_css[256];
                snprintf(scoped_css, sizeof(scoped_css), "%s/main.css", route);
                arwe_server_add_route(s, strdup(scoped_css), views[i].data, views[i].size,
                                      "text/css; charset=utf-8", ARWE_CACHE_IMMUTABLE);
        ...
        } else if (strcmp(views[i].name, "arwe-engine.js") == 0) {
            if (strcmp(route, "/") == 0) {
                arwe_server_add_route(s, "/arwe-engine.js", views[i].data,
                                      views[i].size,
                                      "application/javascript; charset=utf-8",
                                      ARWE_CACHE_IMMUTABLE);
            } else {
                char scoped_bridge[256];
                snprintf(scoped_bridge, sizeof(scoped_bridge), "%s/arwe-engine.js", route);
                arwe_server_add_route(s, strdup(scoped_bridge), views[i].data, views[i].size,
                                      "application/javascript; charset=utf-8", ARWE_CACHE_IMMUTABLE);
            }
        } else if (strcmp(views[i].name, "config.arwe") != 0) {
            ...
            arwe_server_add_route(s, strdup(static_route), views[i].data,
                                  views[i].size, content_type,
                                  ARWE_CACHE_NO_CACHE);
        }
    }

    /* .arweb cru (F3): o browser busca via ARWE.load + verifica CRC */
    char arweb_route[ARWE_UNIT_MAX_NAME + 8];
    snprintf(arweb_route, sizeof(arweb_route), "/%s.arweb", unit->name);
    arwe_server_add_route(s, strdup(arweb_route), arweb, arweb_len,
                          "application/octet-stream", ARWE_CACHE_NO_CACHE);
    return 0;
}
...
static arwe_route_t *find_route(arwe_server_t *s, const char *path,
                                size_t path_len) {
    for (int i = 0; i < s->route_count; i++) {
...
static size_t build_head(char *out, size_t cap, int status,
                         ...
                         size_t body_len, arwe_cache_t cache,
    ...
    int n = snprintf(out, cap,
        ...
        "script-src 'self' 'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval' https://unpkg.com https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://static.cloudflareinsights.com https://challenges.cloudflare.com; "
        ...
        "connect-src 'self' https: http://*.localhost:* http://localhost:* http://127.0.0.1:* https://cloudflareinsights.com https://*.cloudflareinsights.com; "
        "frame-src 'self' https: data: blob:; "
        "frame-ancestors 'self' https://*.alrigroup.com https://*.localhost:* http://*.localhost:*\r\n"
        "%s"
        "Connection: %s\r\n"
        "\r\n",
...
static int etag_of(const uint8_t *data, size_t len, char *out, size_t cap) {
    uint32_t crc = arwe_crc32(data, len);
...
static void handle_client_conn(arwe_server_t *s, int fd) {
    ar_socket_set_recv_timeout(fd, ARWE_SERVER_HEADER_TIMEOUT_MS);
    char buf[ARWE_HTTP_MAX_REQUEST];
    size_t total = 0;
    ...
    while (s->running) {
        ...
        while (total < sizeof(buf) - 1) {
            ...
            if (n > 0) {
                total += (size_t)n;
                buf[total] = '\0';
                int rc = arwe_http_parse(&req, buf, total);
                if (rc == ARWE_HTTP_COMPLETE) {
                    header_complete = 1;
                    break;
                }
                if (rc == ARWE_HTTP_BAD) {
        ...
        if (!header_complete) {
            ...
            size_t hlen = build_head(hbuf, sizeof(hbuf), 400, "Bad Request",
                                     "text/plain", 11, ARWE_CACHE_NO_STORE, 0, NULL);
        ...
        int keep_alive = 0;
        size_t cn_len = 0;
        const char *cn = arwe_http_header(&req, ARWE_HTTP_H_CONNECTION, &cn_len);
        if (cn && cn_len == 10 && memcmp(cn, "keep-alive", 10) == 0) {
            keep_alive = 1;
        }

        /* Method check */
        int is_head = arwe_http_method_is(&req, "HEAD");
        if (!arwe_http_method_is(&req, "GET") && !is_head) {
            char hbuf[4096];
            size_t hlen = build_head(hbuf, sizeof(hbuf), 405, "Method Not Allowed",
                                     "text/plain", 18, ARWE_CACHE_NO_STORE, keep_alive, NULL);
            send_all(fd, hbuf, hlen);
        ...
        /* Match Route */
        arwe_route_t *r = find_route(s, req.path, req.path_len);
        if (!r) {
        ...
        if (!r) {
            if (s->has_notfound_route && s->notfound_route.data) {
                ...
                char hbuf[4096];
                size_t hlen = build_head(hbuf, sizeof(hbuf), 404, "Not Found",
                                         "text/plain", 9, ARWE_CACHE_NO_STORE, keep_alive, NULL);
                send_all(fd, hbuf, hlen);
        ...
        /* ETag match for 304 */
        char etag[16];
        int has_etag = (r->cache == ARWE_CACHE_NO_CACHE && etag_of(r->data, r->size, etag, sizeof(etag)) == 0);
        int not_modified = 0;
        ...
        if (not_modified) {
            size_t hlen = build_head(hbuf, sizeof(hbuf), 304, "Not Modified",
                                     "text/plain", 0, ARWE_CACHE_NO_CACHE,
                                     keep_alive, etag);
            if (send_all(fd, hbuf, hlen) < 0) { ar_socket_close(fd); return; }
        } else {
...
static void *worker_thread_main(void *arg) {
    ...
    while (s->running) {
        ...
        s->queue_head = (s->queue_head + 1) % ARWE_SERVER_QUEUE_SIZE;
        s->queue_count--;
...
int arwe_server_run(arwe_server_t *s, const char *bind, uint16_t port) {
    ...
    if (fd < 0) {
        fprintf(stderr, "[arwe] ar_socket_create failed (code %d)\n", fd);
        return -1;
    }
    ar_socket_reuseaddr(fd, 1);
    int brc = ar_socket_bind(fd, bind, port);
    if (brc < 0) {
        fprintf(stderr, "[arwe] ar_socket_bind to %s:%u failed (code %d)\n", bind, port, brc);
        ar_socket_close(fd);
        return -1;
    }
    int lrc = ar_socket_listen(fd, ARWE_SERVER_LISTEN_BACKLOG);
    if (lrc < 0) {
        fprintf(stderr, "[arwe] ar_socket_listen on port %u failed (code %d)\n", port, lrc);
        ar_socket_close(fd);
        return -1;
    }
    s->listening_fd = fd;
    ...
    for (int i = 0; i < ARWE_SERVER_WORKERS; i++) {
    ...
    printf("[arwe] multi-threaded server listening on %s:%u (%d routes, %d worker threads)\n",
           bind, port, s->route_count, ARWE_SERVER_WORKERS);
    ...
    while (s->running) {
        ...
        ar_mutex_lock(s->queue_mutex);
        if (s->queue_count < ARWE_SERVER_QUEUE_SIZE) {
            s->queue[s->queue_tail].fd = cfd;
            s->queue_tail = (s->queue_tail + 1) % ARWE_SERVER_QUEUE_SIZE;
            s->queue_count++;
            ar_cond_signal(s->queue_cond);
            ar_mutex_unlock(s->queue_mutex);
        } else {
            ...
            size_t hlen = build_head(hbuf, sizeof(hbuf), 503, "Service Unavailable",
                                     "text/plain", 19, ARWE_CACHE_NO_STORE, 0, NULL);
